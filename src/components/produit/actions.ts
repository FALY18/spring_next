	...
	export async function addProduct(product: Omit<Product, 'id'>) {
		...
		console.log("----Produit envoyé au backend:", product);
	...
	export async function deleteProduct(productId: string) {
		try {
		  const response = await fetch(`http://localhost:8080/api/produits/${productId}`, {
		    method: "DELETE",
		  });
	      
		  if (!response.ok) {
		    throw new Error(`Erreur lors de la suppression du produit. Statut: ${response.status}`);
		  }
	      
		  console.log(`Produit avec ID ${productId} supprimé avec succès.`);
		  revalidatePath('/'); // Rafraîchir la page après suppression
		} catch (error) {
	...
	export async function updateProduct(product: any) {
		try {
		const response = await fetch(`http://localhost:8080/api/produits/${product.id}`, {
		method: "PUT",
		headers: {
		"Content-Type": "application/json",
		},
